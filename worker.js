...
export default {
  async fetch(request, env, ctx) {
    ...
    router
      ...
      .post('/v1/image/generations', handleImageGeneration)
      .all('*', handleNotFound);
...
class VisualGPTProvider {
  constructor() {
    ...
    this.IMAGE_UPLOAD_URL = "https://wallpaperaccess.com/full/1556608.jpg";
  ...
  async submitPrediction(prompt, imageUrl = null) {
    ...
    const startingImage = imageUrl && imageUrl.trim() ? imageUrl : this.IMAGE_UPLOAD_URL;
  ...
  async pollStatus(sessionId, timeout = 120000, interval = 5000) {
    ...
    while (true) {
      ...
      headers.path = `/api/v1/prediction/get-status?session_id=${sessionId}`;
